...
start.onclick = async() => {
        ...
        const res = await fetch(`https://codeforces.com/api/user.status?handle=${handle}&lang=${lang}`)
        if (!res.ok) {
            ...
            if (!unsolvedTasks.length) {
                document.getElementById("counter").hidden = false
                document.getElementById("counter").innerHTML = localize[0][`${lang}`].congratsText
                console.log("You haven't got any unsolved tasks :)")
            } else {
                console.log(unsolvedTasks)
                document.getElementById("table-main").hidden = false
                document.getElementById("counter").innerHTML = `${localize[0][`${lang}`].counterOfProblems} ${unsolvedTasks.length}`
            for (let i in unsolvedTasks) {
                let linkToTask = (+unsolvedTasks[i].problem.contestId >= 100000)
                        ? `https://codeforces.com/problemset/gymProblem/${unsolvedTasks[i].problem.contestId}/${unsolvedTasks[i].problem.index}` 
    ...
    document.getElementById("nameTD").innerHTML += '<input type="text" id="searchTask" placeholder="Search...">'
